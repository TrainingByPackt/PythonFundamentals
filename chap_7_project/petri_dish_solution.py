...
# 1. number of each type of cell produced (static methods)
# 2. total number of cells present at a given time step
# 3. total number of iterations performed in simulation
# 4. EXTENSION create a cell that poisons other cells and it shortens
# the poisoned cells life to 4 more iterations. For cells with a life span
# less than 4 iterations this does nothing.
# 5. Need a method for completely resetting the class count.
...
def activity1_task2():
    ...
    petri_dish2 = PetriDish(30)
    petri_dish2.add_to_world(Cell(position=(10, 10)))
    petri_dish2.begin_simulation()

    petri_dish3 = PetriDish(30)
    petri_dish3.add_to_world(Cell(position=(5, 5)))
    petri_dish3.add_to_world(Cell(position=(10, 10)))
    petri_dish3.add_to_world(Cell(position=(0, 0)))
    petri_dish3.add_to_world(Cell(position=(15, 15)))
    petri_dish3.begin_simulation()
...
def final_run():
    ...
    for _ in range(NUM_OF_GENERIC):
        cell = Generic(position=choice(world.available_positions))
        world.add_to_world(cell)
    for _ in range(NUM_OF_ECOLI):
        cell = Ecoli(position=choice(world.available_positions))
        world.add_to_world(cell)
    for _ in range(NUM_OF_HUNTER_KILLERS):
        cell = HunterKiller(position=choice(world.available_positions))
        world.add_to_world(cell)
    for _ in range(NUM_OF_BICOLI):
        cell = Structure(position=choice(world.available_positions))
        world.add_to_world(cell)

    world.begin_simulation(delay=0.01, movement=True)
